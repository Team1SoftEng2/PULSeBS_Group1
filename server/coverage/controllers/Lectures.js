...
module.exports.apiDelectedLecturesListGET = async (req, res) => {
  const [err, lectures] = await to(Lectures.getLecturesDelectList(req.query.courseId, req.query.time));
  if (err) {
    return utils.writeJson(res, {errors: [{'param': 'Server', 'msg': err}]}, 500);
  }
  return utils.writeJson(res, lectures);
};
...
module.exports.deadlineNotification = async function() {
  const [err, lectures] = await to(Lectures.getLectures());
  if (err) return {err: 'Failed to communicate with the DB'};

  const sent = await Promise.all(lectures.map(async (l) => {
